...
$MENU_TOGGLE.on('click', function() {
		...
		if ($BODY.hasClass('nav-md')) {
			...
			$('.navbar_title').css("margin-left", "0px");
		} else {
			$SIDEBAR_MENU.find('li.active-sm ul').show();
			$SIDEBAR_MENU.find('li.active-sm').addClass('active').removeClass('active-sm');
			$('.navbar_title').css("margin-left", "240px");
		}
...
if (typeof NProgress != 'undefined') {
			...
			if ($('#brazil_map').length ){
				...
				$('#brazil_map').vectorMap({
					...
					onLabelShow: function(event, label, code){
						/*console.log(label[0].innerText + crabData[code]);*/
						if (crabData[code] == undefined) {crabData[code] = 0}
						label.html('<div style="font-size:16px; padding:2px;"><b>' + label[0].innerText + '</b><br>Number of uses: ' + crabData[code] + '</div>'); 
					}
				});
				...
				// Fill with gradient
				ctx.fillStyle = grd;
				ctx.fillRect(10,10,25,cHeight);

				ctx.fillStyle = "black";
				ctx.font = "15px Arial";
				var median = (max+min)/2;
				ctx.fillText(roundToFive(max),45,25);
				ctx.fillText(roundToFive((median+max)/2),45,(25+((cHeight+25)/2))/2);
				ctx.fillText(roundToFive(median),45,(cHeight+25)/2);
				ctx.fillText(roundToFive((median+min)/2),45,(cHeight+((cHeight+25)/2))/2);
				ctx.fillText(roundToFive(min),45,cHeight);
	...
	n = n.toFixed(0);
    var n2 = n.substring(0, n.length-1) + '0';
    n = Number(n);
    n2 = Number(n2);
    var diff = n-n2;
    if (diff > 4) {
	...
	/* CROPPER */
	   ...
	   /* DATERANGEPICKER */
		...
		function init_daterangepicker() {
			...
			getDaterange($('#reportrange').data('daterangepicker'), -1);
	   ...
	   function init_daterangepicker_right() {
				...
				if( typeof ($.fn.daterangepicker) === 'undefined'){ return; }
				if( $('#reportrange_right').length === 0){ return; }
				...
				getDaterange($('#reportrange_right').data('daterangepicker'), 1);

				$('#reportrange_right span').html($('#reportrange_right').data('daterangepicker').startDate.format('MMMM D, YYYY') + ' - ' + moment().format('MMMM D, YYYY'));
	    ...
	    function init_daterangepicker_single_call() {
				...
				if( typeof ($.fn.daterangepicker) === 'undefined'){ return; }
				if( $('#reportrange_left').length === 0){ return; }
				//console.log('init_daterangepicker_left');
		  
				var cb = function(start, end, label) {
				  //console.log(start.toISOString(), end.toISOString(), label);
				  $('#reportrange_left span').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY'));
				};
				...
				$('#reportrange_left span').html($('#reportrange_left').data('daterangepicker').startDate.format('MMMM D, YYYY') + ' - ' + moment().format('MMMM D, YYYY'));

				getDaterange($('#reportrange_left').data('daterangepicker'), 0);
					...
					},
					{
						...
						customize: function (csv) {
							...
							$('#datatable-buttons th').each(function(i){
								...
								{
									headers += $(this).html().split("<select>")[0] + ',';
								}
								else if ($(this).html().includes("<input"))
								{
									headers += $(this).html().split("<input")[0]+','
								}
								
							});
							var tempH = headers.split(',');
							tempH.splice(0,1);
							split_csv[0] = tempH.join(',');

							var order = ["Species", "Andada Year", "Andada Month", "Moon Phase", "State", "Profession", "Submission Date", "Version", "User ID", "Prominent Day", "Another Andanda", "Time of Day", "Berried Females", "Habitat", "County", "Protected Area", "Local Name", "Additional Observations", "Date Range"];

							var colIndex = [];

							var headSplit = headers.split(',');
							var habitatIndex = -1;
							...
							for (i in order)
							{
								for (h in headSplit)
								{
									if (order[i] == headSplit[h])
									{
										colIndex.push(i);
									}
									if (headSplit[h]=='Habitat')
									{
										habitatIndex = (h-1);
									}
								}
							...
							$.each(split_csv.slice(1), function (index, csv_row) {
								...
								for(i in colIndex)
								{
									newArray.push(csv_cell_array[colIndex[i]]);
								}
								if (headers.includes("Date Range"))
								{
									var newLine = '"' + newArray.join('","');
								}
								else
								{
									var newLine = '"' + newArray.join('","') + '"';
								}
								split_csv[index + 1] = newLine;
							});
							...
							if (habitatIndex != -1)
							{
								$.each(split_csv.slice(1), function (index, csv_row) {
	                                ...
	                                var csv_cell_array = csv_row.split('","');
	                                var temp = csv_cell_array[habitatIndex].split(' ');
	                                var final = ''
	                                for (i in temp){
	                                	if (temp[i] != ''){
	                                		final += temp[i] + '; ';
									...
									csv_cell_array[habitatIndex] = final.substring(0, final.length-2);
						...
						this.api().columns().every( function () {
								...
								else if (column[0][0] === 4) // moon phase has special look-up
								{
									...
									var select = $('<select><option value=""></option></select>')	
										...
										select.append( '<option value="Full">Full</option>' );
										select.append( '<option value="New">New</option>' );
										select.append( '<option value="Anormalous">Anormalous</option>' );
								}
								...
								else if (column[0][0] === 14)  // Habitat
								{
									...
									select.append( '<option value="Leito de rio">Leito de rio</option>' );
									select.append( '<option value="Costão rochoso">Costão rochoso</option>' );
								...
								else if (column[0][0] === 18) // additional obs
								{
									...
									.on( 'change', function () {
											...
											else if(val == "Coded Observer"){
											/* Note, to use regex search, you need the last two params to be set to true,false *
											 * and you need to double escape characters (for instance \d{1} becomes \\d{1})    *
											 * This search below searches for any instance of RC followed by 4 digits          */
												column
												.search('[R|r][C|c]\\d{4}',true,false)
												.draw();
											}
											else
											{
								...
								select.append('<option value="Coded Observer">Coded Observer</option>');
